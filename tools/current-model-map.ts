...
function parseExpression(source: ts.SourceFile, expression: ts.Expression): ParsedValue {
  ...
  switch (expression.kind) {
    ...
    case ts.SyntaxKind.ArrayLiteralExpression:
      return (expression as ts.ArrayLiteralExpression).elements.map((el) => parseExpression(source, el));
    default:
...
function parseSchemaFile(filePath: string): ParsedCollection[] {
  ...
  source.forEachChild((node) => {
    ...
    const collectionName =
      schemaDecorator && typeof schemaDecorator["collection"] === "string" ? schemaDecorator["collection"] : defaultCollectionName(className);
    collections.push({
      name: className,
